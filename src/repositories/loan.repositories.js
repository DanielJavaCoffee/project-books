...
function findByIdLoansRepository(id) {
    return new Promise((resolve, reject) => {
...
function deleteLoansRepository(id) {
    return new Promise((resolve, reject) => {
        db.run(`DELETE FROM loans WHERE id = ?`, [id], function(err) {
            if(err) {
                reject(err)
            } else {
                resolve({message: 'Loan deleted successfully', id})
            }
        })
    })
}
...
export default {
    ...
    findByIdLoansRepository,
    deleteLoansRepository
}