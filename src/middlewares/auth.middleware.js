...
export function authMiddleware(req, res, next) {
    ...
    if(!tokenHeader) return res.status(401).send({message: 'The token was not informed!'});

    const partsToken = tokenHeader.split(' ');
    if(partsToken.length !== 2) return res.status(401).send({message: 'The token was not informed!'});
    ...
    jwt.verify(token, process.env.SECRET_JWT, async (err, decoded) => {
        if(err) return res.status(401).send({message: 'Invalid Token!', error: err.message});
